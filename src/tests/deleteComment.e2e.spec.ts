...
describe("DELETE /comments/:id", () => {
  ...
  it("should return 204 and delete comment by id", async () => {
    await request(app)
      .delete(`/comments/${createdCommentId}`)
      .set("Authorization", `Bearer ${accessToken1}`)
      .expect(204);

    await request(app).get(`/comments/${createdCommentId}`).expect(404);
  });
  ...
  it("should return 401 if Authorization header not set", async () => {
    ...
    await request(app).delete(`/comments/${createdCommentId}`).expect(401);
  });

  it("should return 404 if comment not exist", async () => {
    await request(app)
      .delete(`/comments/66d1f5a8c3e7429b8f1a6d30`)
      .set("Authorization", `Bearer ${accessToken1}`)
      .expect(404);
  });

  it("should return 403 if trying to delete comment not owner", async () => {
    await request(app)
      .delete(`/comments/${createdCommentId}`)
